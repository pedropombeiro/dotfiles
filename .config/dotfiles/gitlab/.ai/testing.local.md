...
- Declare RSpec subjects on `describe` blocks, not inside `context` blocks; vary inputs with `let`.
...
- Keep setup and stubbing out of RSpec examples. Put scenario setup in a `context` using `let` values and a `before` hook; reserve local variables in examples for test-specific inputs, then exercise behavior and assert outcomes.
